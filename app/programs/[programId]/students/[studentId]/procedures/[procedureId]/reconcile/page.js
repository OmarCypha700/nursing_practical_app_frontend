...
export default function ReconcilePage() {
  ...
  useEffect(() => {
    ...
    api
      .get(`/exams/students/${studentId}/procedures/${procedureId}/reconciliation/`)
      .then((res) => {
        setReconciliationData(res.data);
        
        // Initialize final scores with examiner_a scores as default
        const initialScores = {};
        res.data.steps.forEach((step) => {
          initialScores[step.id] = step.examiner_a_score ?? 0;
        });
  ...
  const handleSubmit = async () => {
    ...
    const allStepsScored = reconciliationData.steps.every(
      (step) => finalScores[step.id] !== undefined && finalScores[step.id] !== null
    );

    if (!allStepsScored) {
      alert("Please provide a score for all steps before submitting.");
      return;
    }
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {reconciliationData.student.full_name} - {reconciliationData.student.index_number}
            </p>
          </div>
        </div>
        ...
        <table className="w-full table-auto">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="text-center p-4 font-semibold w-48">Final Score</th>
            </tr>
          </thead>
          <tbody>
            {reconciliationData.steps.map((step, index) => {
              const scoresMatch = step.examiner_a_score === step.examiner_b_score;
                  ...
                  <td className="p-4 text-center">
                    <span
                      className={`inline-flex items-center justify-center w-10 h-10 rounded-full font-bold ${
                        scoresMatch ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"
                      }`}
                    >
